...
const test = async () => {
  try {
    const register = async (u: string, e: string) => {
    ...
    // Promote u1 to celebrity manually for testing
    await prisma.user.update({ where: { id: id1 }, data: { isCelebrity: true } });
    ...
    console.log('Has celebrity post?', feed.data.items.some((i: any) => i.id === post.data.id));
